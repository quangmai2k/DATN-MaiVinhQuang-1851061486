...
class DanhSachTTSXuatCanh extends StatelessWidget {
  const DanhSachTTSXuatCanh({Key? key}) : super(key: key);
  @override
  Widget build(BuildContext context) {
    return HeaderAndNavigation(widgetBody: DanhSachTTSXuatCanhBody());
  }
}

class DanhSachTTSXuatCanhBody extends StatefulWidget {
  const DanhSachTTSXuatCanhBody({Key? key}) : super(key: key);

  @override
  State<DanhSachTTSXuatCanhBody> createState() =>
      _DanhSachTTSXuatCanhBodyState();
}

class _DanhSachTTSXuatCanhBodyState extends State<DanhSachTTSXuatCanhBody> {
  String? request;
  ...
  getListTrainee(page) async {
    ...
    if (queryRequest != '' && dateFrom != null && dateTo != null) {
      queryRequest +=
          " and departureDate>:'$dateFrom' and departureDate<:'$dateTo'";
    } else if (queryRequest != '' && dateFrom != null && dateTo == null) {
      queryRequest += " and departureDate>:'$dateFrom'";
    } else if (queryRequest != "" && dateFrom == null && dateTo != null) {
      queryRequest += " and departureDate<:'$dateTo'";
    } else if (queryRequest == "" && dateFrom != null && dateTo != null) {
      queryRequest += "departureDate>:'$dateFrom' and departureDate<:'$dateTo'";
    } else if (queryRequest == "" && dateFrom == null && dateTo != null) {
      queryRequest += "departureDate<:'$dateTo'";
    } else if (queryRequest == "" && dateFrom != null && dateTo == null) {
      queryRequest += "departureDate>:'$dateFrom'";
    }
    if (queryRequest != '') {
      queryRequest = 'and $queryRequest';
    }
    print(queryRequest);
    if (request == null && (selectedDH == "" || selectedDH == "0")) {
      response = await httpGet(
          "/api/nguoidung/get/page?page=${page - 1}&size=$rowPerPage&sort=id&filter=isTts:1 and ttsStatusId in (10,11)  and (stopProcessing:0 or stopProcessing is null) $queryRequest",
          context);
    } else {
      if (request != null && (selectedDH == "" || selectedDH == "0")) {
        response = await httpGet(
            "/api/nguoidung/get/page?page=${page - 1}&size=$rowPerPage&sort=id&filter=(fullName~'*$request*' or userCode~'*$request*')$trangThaiTTS  and (stopProcessing:0 or stopProcessing is null) $queryRequest",
            context);
      } else {
        if (request == null && (selectedDH != "" && selectedDH != "0")) {
          response = await httpGet(
              "/api/nguoidung/get/page?page=${page - 1}&size=$rowPerPage&filter=orderId:$selectedDH AND isTts:1 $trangThaiTTS and (stopProcessing:0 or stopProcessing is null) $queryRequest",
              context);
        } else {
          if (request != null && (selectedDH != "" || selectedDH != "0")) {
            response = await httpGet(
                "/api/nguoidung/get/page?page=${page - 1}&size=$rowPerPage&filter=orderId:$selectedDH AND (fullName~'*$request*' or userCode~'*$request*') AND isTts:1 $trangThaiTTS and (stopProcessing:0 or stopProcessing is null) $queryRequest",
                context);
          }
        }
      }
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<dynamic>(
      future: userRule('/danh-sach-tts-xuat-canh', context),
                          ...
                          child: Column(
                            ...
                            children: [
                              TitlePage(
                                ...
                                content: 'Danh sách thực tập sinh xuất cảnh',
                              ),
                            ],
                          ),
                        ),
                                      ...
                                      columns: [
                                        ...
                                        DataColumn(
                                            label: Text('SĐT',
                                                style: titleTableData)),
                                        DataColumn(
                                            label: Text('Cán bộ tuyển dụng',
                                                style: titleTableData)),
                                        DataColumn(
                                            label: Text('Trạng thái',
                                                style: titleTableData)),
                                        DataColumn(
                                            label: Text('Ngày xuất cảnh',
                                                style: titleTableData)),
                                        DataColumn(
                                          label: Text(' '),
                                        ),
                                                    ...
                                                    !widget.tableData[i][
                                                        ...
                                                        ? Row(
                                                            ...
                                                            children: [
                                                              ...
                                                              Text(
                                                                  "${widget.tableData[i]["fullName"] ?? ""} (${widget.tableData[i]["userCode"] ?? ""})",
                                                                  style:
                                                                      bangDuLieu),
                                                            ],
                                                          )
