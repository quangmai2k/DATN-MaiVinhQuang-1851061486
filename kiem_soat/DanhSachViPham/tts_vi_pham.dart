...
class TTSViPham extends StatefulWidget {
  const TTSViPham({Key? key}) : super(key: key);

  @override
  State<TTSViPham> createState() => _TTSViPhamState();
}

class _TTSViPhamState extends State<TTSViPham> {
  ...
  Future<void> exportExcel(result) async {
    ...
    sheet.getRangeByName('A3').setText('Danh sách TTS vi phạm');
    ...
    sheet.getRangeByName('D5').setText('Ngày sinh');
    sheet.getRangeByName("E5").setText('Cán bộ tuyển dụng');
    ...
    for (int i = 0; i < result.length; i++) {
      ...
      sheet.getRangeByIndex(6 + i, 4).setText("${result[i]["nguoidung"]["birthDate"]}");
  ...
  Future getListDSViPham(page, String findSearch) async {
    ...
    var response;
    if (findSearch == "") {
      response = await httpGet("/api/quyetdinh-xuphat-chitiet/get/page?page=$page&size=$rowPerPage&sort=id&filter=nguoidung.isTts:1", context);
    } else {
      response = await httpGet("/api/quyetdinh-xuphat-chitiet/get/page?page=$page&size=$rowPerPage&filter=nguoidung.isTts:1 $findSearch", context);
    }
                                ...
                                child: Container(
                                  child: Row(
                                    children: [
                                      Expanded(
                                        flex: 2,
                                        child: Text('TTS', style: titleWidgetBox),
                                      ),
                                      Expanded(
                                        flex: 5,
                                        child: Container(
                              ...
                              Container(
                                ...
                                child: TextButton(
                                  ...
                                  onPressed: () {
                                    ...
                                    var denNgay;
                                    if (_tts.text != "")
                                      tts = "and nguoidung.fullName~'*${_tts.text}*' ";
                                    else
                                      tts = "";
                                    if (time1 != null)
                                ...
                                child: TextButton(
                                  ...
                                  onPressed: () async {
                                    ...
                                    Future getListDSViPham1(fromDate, toDate) async {
                                      ...
                                      response = await httpGet("/api/quyetdinh-xuphat-chitiet/get/page?filter=nguoidung.isTts:1$condition", context);

                                      if (response.containsKey("body")) {
                                        setState(() {
                                          resultDSVP1 = jsonDecode(response["body"])['content'];
                                          //totalElements = resultDSVP1["totalElements"];
                                          // print("resultDSVP1:$resultDSVP1");
                                        });
                                      }
                                      return resultDSVP1;
                                    }
                              ...
                              Expanded(
                                ...
                                child: DataTable(
                                  columns: <DataColumn>[
                                    ...
                                    DataColumn(label: Text("Cán bộ \ntuyển dụng", style: titleTableData)),
                                    ...
                                    for (int i = 0; i < resultDSVP["content"].length; i++)
                                      DataRow(cells: [
                                        DataCell(Container(
                                            ...
                                            child: Text("${tableIndex + i}", style: bangDuLieu))),
                                        DataCell(Container(
                                            width: (MediaQuery.of(context).size.width / 10) * 0.65,
                                            child: Text(resultDSVP["content"][i]["nguoidung"]["userCode"] ?? "no data", style: bangDuLieu))),
                                        DataCell(Container(
                                            width: (MediaQuery.of(context).size.width / 10) * 0.65,
                                            child: Text(resultDSVP["content"][i]["nguoidung"]["fullName"] ?? "no data", style: bangDuLieu))),
                                        DataCell(Container(
                                            width: (MediaQuery.of(context).size.width / 10) * 0.65,
                                            child: Text((resultDSVP["content"][i]["nguoidung"]["birthDate"] !=  null)
                                                ?FormatDate.formatDateView(DateTime.parse(resultDSVP["content"][i]["nguoidung"]["birthDate"]!)):'',
                                                style: bangDuLieu))),
                                        DataCell(Container(
                                          width: (MediaQuery.of(context).size.width / 10) * 0.65,
                                          child: Text(
                                              resultDSVP["content"][i]["nhanvientuyendung"] != null
                                                  ? resultDSVP["content"][i]["nhanvientuyendung"]["fullName"]
                                                  : "no data",
                                              style: bangDuLieu),
                                        )),
                                        DataCell(Container(
                                            width: (MediaQuery.of(context).size.width / 10) * 0.65,
                                            child:
                                                Text(resultDSVP["content"][i]["quyetdinh"]["quydinh"]["ruleName"] ?? "no data", style: bangDuLieu))),
                                        DataCell(Text(
                                            (resultDSVP["content"][i]["quyetdinh"]["decisionDate"] != null)
                                                ? DateFormat('dd/MM/yyyy')
                                                    .format(DateTime.parse(resultDSVP["content"][i]["quyetdinh"]["decisionDate"] ?? "no data"))
                                                : "",
                                            style: bangDuLieu)),
