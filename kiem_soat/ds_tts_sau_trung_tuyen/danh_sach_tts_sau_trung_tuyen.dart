...
import '../../ho_so_noi/chot_danh_sach_tts_tien_cu/dung_xu_ly.dart';
...
class DanhSachTTSSauTrungTuyen extends StatelessWidget {
  const DanhSachTTSSauTrungTuyen({Key? key}) : super(key: key);
  @override
  Widget build(BuildContext context) {
    return HeaderAndNavigation(widgetBody: DanhSachTTSSauTrungTuyenBody());
  }
}

class DanhSachTTSSauTrungTuyenBody extends StatefulWidget {
  const DanhSachTTSSauTrungTuyenBody({Key? key}) : super(key: key);

  @override
  State<DanhSachTTSSauTrungTuyenBody> createState() =>
      _DanhSachTTSSauTrungTuyenBodyState();
}

class _DanhSachTTSSauTrungTuyenBodyState
    extends State<DanhSachTTSSauTrungTuyenBody> {
  String? request;
  ...
  Future<List<Order>> getListOrder() async {
    ...
    var response1 = await httpGet(
        "/api/donhang/get/page?sort=id&(stopProcessing:0 or stopProcessing is null)",
        context);
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<dynamic>(
      future: userRule('/danh-sach-tts-sau-khi-trung-tuyen', context),
                  ...
                  return Consumer<NavigationModel>(
                    builder: (context, navigationModel, child) => ListView(
                      children: [
                        TitlePage(
                          listPreTitle: [
                            {'url': '/ho-so-noi', 'title': 'Dashboard'},
                          ],
                          content: 'Danh sách thực tập sinh sau trúng tuyển',
                        ),
                        Container(
